...
export default function Table() {
    ...
    const sort = (fn: (a:any, b:any) => number) => {
        const sortedData = results.advertiserDomains.sort(fn);
        setSortOrder(prev => prev === 'asc' ? 'desc' : 'asc')
        setResults(prev => ({
            ...prev,
            advertiserDomains: sortedData
        }))
    };
    ...
    if (searchTerm.length && searchResults.length == 0) {
        return (
    ...
    return (
        <table className="w-full text-sm border-separate">
            <thead className="sticky top-[126px] md:top-[90px] z-10">
                <tr>
                    <th onClick={() => sort((a, b) => sortOrder === 'asc' ? a.domain.localeCompare(b.domain) : b.domain.localeCompare(a.domain))} className="clickable pl-8">Domain</th>
                    <th onClick={() => sort((a, b) => sortOrder === 'asc' ? b.count - a.count : a.count - b.count)} className="clickable w-1/3">Count</th>
                </tr>
            </thead>
            <tbody>
                {(searchResults.length ? searchResults : results.advertiserDomains).map(({domain, count}) => (
                    <tr className="hover:bg-gray-50" key={domain}>
                        <td className="pl-8">{domain}</td>
                        <td>{count}</td>
                    </tr>
                ))}
            </tbody>
